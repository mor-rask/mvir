import Tkinter, thread, os, time, win32api
...
class C4SMVIRT:
    ...
    def player(self):
        ...
        while True:
            try:
                ...
                if self.frame == 1:
                    thread.start_new_thread(self.tk_widget, ())
                elif self.frame_count == (self.song_length):
                    if self.song != self.player_stack()[0] - 1:
                        self.song_length = self.frame + (self.wf.getnframes() / self.chunk_size)
                        self.song += 1
                    else:
                        self.song = 0
                        self.song_length = self.frame + (self.wf.getnframes() / self.chunk_size)
    ...
    def tk_widget(self):
        ...
        # 1280x1024 monitor
        root.geometry('+%s+%s' % (str(win32api.GetSystemMetrics(0)/2 + 160), str(win32api.GetSystemMetrics(1)/2+291)))
        root.configure()
        ...
        thread.start_new_thread(drag_bind, ())
        ...
        thread.start_new_thread(frame_rate_update, ())
  ...
  \_____|  |_|   |___/ |_| |_| |_|\/   |_|_|   \__| v1.1
    ...
    Wanna change the default configurations?
        ...
        if o1 in ['y', 'Y', 'Yes', 'yes', 'YES']:
            ...
            print bg_color_list
            self.plot_bg = raw_input('Set BACKGROUND color:')
            print line_color_list
            self.line_fg = raw_input('Set LINE color:')
            print line_width_list
            self.line_width = float(raw_input('Set LINE width:'))
            print chunk_size_list
            print('The default value is 256')
            self.chunk_size = int(raw_input('Set CHUNK size:'))
            self.player()
